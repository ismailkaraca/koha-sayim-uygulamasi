...
const RobustBarcodeScanner = ({ onScan, onClose, isPaused }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 z-40 flex flex-col items-center justify-center p-4">
            ...
            <button onClick={onClose} className="mt-4 px-6 py-2 bg-gray-700 text-white rounded-md font-bold hover:bg-gray-800">Kamerayı Kapat</button>
        </div>
    );
};
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-gray-100">
            <div className="max-w-lg w-full p-8 bg-white rounded-xl shadow-lg text-center space-y-6">
                <h1 className="text-3xl font-bold text-gray-800">Kamera Erişimi</h1>
                
                {step === 'initial' && (
                    <>
                        <p className="text-gray-600">Kamera ile sayım yapmak istiyor musunuz?</p>
...
// --- Header Component ---
const Header = ({ page, setPage, currentSessionName, selectedLibrary, kohaData }) => {
    const navItems = [
        { id: 'start', label: '1. Yeni Sayım', disabled: false },
        { id: 'setup', label: '2. Kurulum', disabled: !currentSessionName },
        { id: 'scan', label: '3. Sayım', disabled: !selectedLibrary || kohaData.length === 0 },
        { id: 'summary', label: '4. Özet & Raporlar', disabled: !selectedLibrary || kohaData.length === 0 }
    ];
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-16">
                    <div className="flex items-center">
                        ...
                        <h1 className="ml-3 text-xl font-bold text-gray-800 hidden sm:block">Koha Çevrimdışı Sayım</h1>
                    </div>
                    <nav className="hidden md:flex md:items-center md:space-x-2">
                        {navItems.map(item => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 ease-in-out transform hover:scale-105 ${
                                    page === item.id
                                        ? 'bg-blue-600 text-white shadow-sm'
                                        : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'
                                } ${item.disabled ? 'opacity-40 cursor-not-allowed hover:scale-100' : ''}`}
                            >
                                {item.label}
                            </button>
                        ))}
...
export default function App() {
    ...
    // --- Render Functions ---
    const renderStartScreen = () => <div className="max-w-4xl w-full p-8 bg-white rounded-xl shadow-lg space-y-8">
    ...
    const renderScanScreen = () => <>{isCameraOpen && isQrCodeReady && isCameraAllowed && <RobustBarcodeScanner onClose={() => setIsCameraOpen(false)} onScan={handleCameraScan} isPaused={warningModal.isOpen} />} <div className="flex flex-col md:flex-row h-full bg-gray-50">
        ...
        <button onClick={() => setIsCameraOpen(true)} disabled={!isQrCodeReady || !isCameraAllowed} className="w-full flex items-center justify-center gap-2 p-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:bg-gray-400 disabled:cursor-not-allowed">
