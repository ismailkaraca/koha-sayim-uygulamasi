...
export default function App() {
    ...
    // --- Render Functions ---
    const renderStartScreen = () => <div className="max-w-4xl w-full p-8 bg-white rounded-xl shadow-sm space-y-8">
        <div className="text-center"><h1 className="text-4xl font-bold text-slate-800">Koha Çevrimdışı Sayım</h1><p className="text-slate-600 mt-2">Yeni bir sayım başlatın veya kayıtlı bir oturuma devam edin.</p></div>
        <div className="p-6 border border-slate-200 rounded-lg"><h2 className="text-2xl font-semibold mb-4 text-slate-700">Yeni Sayım Başlat</h2><div className="flex flex-col sm:flex-row gap-2"><input type="text" value={sessionNameInput} onChange={e => {setSessionNameInput(e.target.value); setError('')}} onKeyDown={(e) => e.key === 'Enter' && startNewSession()} placeholder="Yeni sayım için bir isim girin (örn: Zemin Kat Romanlar)" className="flex-grow p-3 border border-slate-300 rounded-md shadow-sm focus:ring-slate-500 focus:border-slate-500" /><button onClick={startNewSession} className="bg-slate-700 text-white font-bold py-3 px-6 rounded-md hover:bg-slate-800 transition-colors">Başlat</button></div></div>
        <div className="p-6 border border-slate-200 rounded-lg"><h2 className="text-2xl font-semibold mb-4 text-slate-700">Kayıtlı Oturumlar</h2>{Object.keys(sessions).length > 0 ? <ul className="space-y-3">{Object.values(sessions).sort((a,b) => new Date(b.lastUpdated) - new Date(a.lastUpdated)).map(session => <li key={session.name} className="flex flex-col sm:flex-row items-center justify-between p-3 bg-slate-50 rounded-lg border"><div><p className="font-bold text-slate-800">{session.name}</p><p className="text-sm text-slate-500">{new Date(session.lastUpdated).toLocaleString('tr-TR')} - {session.items.length} kayıt</p></div><div className="flex gap-2 mt-2 sm:mt-0"><button onClick={() => loadSession(session.name)} className="px-4 py-2 text-sm bg-green-600 text-white rounded hover:bg-green-700">Yükle</button><button onClick={() => deleteSession(session.name)} className="px-4 py-2 text-sm bg-red-600 text-white rounded hover:bg-red-700">Sil</button></div></li>)}</ul> : <p className="text-slate-500">Kayıtlı oturum bulunamadı.</p>}</div>
    </div>;

    const renderSetupScreen = () => <div className="max-w-2xl w-full p-8 bg-white rounded-xl shadow-sm space-y-6">
        <h1 className="text-3xl font-bold text-slate-800">Sayım Kurulumu: "{currentSessionName}"</h1>{error && <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded" role="alert"><p>{error}</p></div>}<div className="space-y-4"><div><label htmlFor="library-select" className="block text-sm font-medium text-slate-700 mb-1">Kütüphanenizi Seçin</label><div className="flex gap-2"><select id="library-select" value={selectedLibrary} onChange={(e) => setSelectedLibrary(e.target.value)} className="w-full p-3 border border-slate-300 rounded-md shadow-sm"><option value="">-- Kütüphane Seçiniz --</option>{Object.entries(combinedLibraries).map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select><button onClick={()=> setAddDataModal({isOpen: true, type: 'library'})} className="px-3 bg-slate-200 rounded-md hover:bg-slate-300">Yeni Ekle</button></div></div><div><label htmlFor="location-select" className="block text-sm font-medium text-slate-700 mb-1">Bölüm/Materyalin Yeri (Opsiyonel)</label><div className="flex gap-2"><select id="location-select" value={selectedLocation} onChange={(e) => setSelectedLocation(e.target.value)} className="w-full p-3 border border-slate-300 rounded-md shadow-sm"><option value="">-- Tüm Lokasyonlar --</option>{Object.entries(combinedLocations).map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select><button onClick={()=> setAddDataModal({isOpen: true, type: 'location'})} className="px-3 bg-slate-200 rounded-md hover:bg-slate-300">Yeni Ekle</button></div><p className="text-xs text-slate-500 mt-1">Yer seçimi yaparsanız, sayım yaptığınız yerde olmayan materyallerle ilgili uyarı verilecektir.</p></div></div><div><h3 className="text-sm font-medium text-slate-700 mb-1">Koha'dan Aldığınız Sayım İçin Hazırlanmış Dosya (.xlsx)</h3><FileUploader onFileAccepted={handleExcelUpload} title={kohaData.length > 0 ? `${kohaData.length} kayıt yüklendi.` : "Dosyayı buraya sürükleyin veya seçmek için tıklayın"} disabled={!isXlsxReady}><svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 text-green-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg></FileUploader>{isLoading && <p className="text-blue-600 mt-2">Dosya okunuyor...</p>}</div><button onClick={() => { if(selectedLibrary && kohaData.length > 0) setPage('scan'); else setError("Sayıma başlamak için Kütüphane seçmeli ve Excel dosyası yüklemelisiniz."); }} disabled={!selectedLibrary || kohaData.length === 0 || isLoading || !isXlsxReady} className="w-full bg-slate-700 text-white font-bold py-3 px-4 rounded-md hover:bg-slate-800 disabled:bg-slate-400">Sayıma Devam Et</button><button onClick={() => setPage('start')} className="w-full mt-2 bg-slate-500 text-white font-bold py-2 px-4 rounded-md hover:bg-slate-600">Geri</button>
    </div>;
        ...
        <div className="bg-slate-100 min-h-screen font-sans flex">
            ...
            <main className={`flex-1 ${page !== 'permission' ? 'md:ml-64' : ''}`}>
                {page === 'permission' ? (
                    ...
                    <div className="flex justify-center w-full p-4 sm:p-6 md:p-8">
                       {renderPageContent()}
                    </div>
                )}
            </main>
